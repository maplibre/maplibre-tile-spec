...
public class GeometryEncoder {
    ...
    public static Triple<Integer, byte[], Integer> encodeGeometryColumn(List<Geometry> geometries, PhysicalLevelTechnique physicalLevelTechnique,
        ...
        * */
        ...
        var encodedMortonVertexDictionary = IntegerEncoder.encodeMortonCodes(new ArrayList<>(mortonEncodedDictionary), physicalLevelTechnique);
        ...
        if(numGeometries.size() == 0 && numRings.size() == 0 && featureIds.size() > 0 &&  numParts.size() == featureIds.size()){
        ...
        var encodedTopologyStreams = IntegerEncoder.encodeIntStream(geometryTypes, physicalLevelTechnique, false,
                PhysicalStreamType.LENGTH, null);
        var numStreams = 1;
        if(numGeometries.size() > 0){
            var encodedNumGeometries = IntegerEncoder.encodeIntStream(numGeometries, physicalLevelTechnique, false,
                    PhysicalStreamType.LENGTH, new LogicalStreamType(LengthType.GEOMETRIES));
            encodedTopologyStreams = ArrayUtils.addAll(encodedTopologyStreams, encodedNumGeometries);
            numStreams++;
        }
        if(numParts.size() > 0){
            var encodedNumParts = IntegerEncoder.encodeIntStream(numParts, physicalLevelTechnique, false,
                    PhysicalStreamType.LENGTH, new LogicalStreamType(LengthType.PARTS));
            encodedTopologyStreams = ArrayUtils.addAll(encodedTopologyStreams, encodedNumParts);
            numStreams++;
        }
        if(numRings.size() > 0){
            var encodedNumRings = IntegerEncoder.encodeIntStream(numRings, physicalLevelTechnique, false,
                    PhysicalStreamType.LENGTH, new LogicalStreamType(LengthType.RINGS));
        ...
        var dictionaryEncodedSize = encodedDictionaryOffsets.encodedValues.length + encodedVertexDictionary.encodedValues.length;
        var mortonDictionaryEncodedSize = encodedMortonEncodedDictionaryOffsets.encodedValues.length + encodedMortonVertexDictionary.encodedValues.length;
        if(plainVertexBufferSize <= dictionaryEncodedSize && plainVertexBufferSize <= mortonDictionaryEncodedSize){
            //TODO: get rid of extra conversion
            var encodedVertexBufferStream = encodeVertexBuffer(Arrays.stream(zigZagDeltaVertexBuffer).boxed().collect(Collectors.toList()),
                    physicalLevelTechnique);
            numStreams++;
            return Triple.of(numStreams, ArrayUtils.addAll(encodedTopologyStreams, encodedVertexBufferStream), maxVertexValue);
        }
        else if(dictionaryEncodedSize < plainVertexBufferSize && dictionaryEncodedSize <= mortonDictionaryEncodedSize){
            var encodedVertexOffsetStream = IntegerEncoder.encodeIntStream(dictionaryOffsets, physicalLevelTechnique,
                    false, PhysicalStreamType.OFFSET, new LogicalStreamType(OffsetType.VERTEX));
            var encodedVertexDictionaryStream = encodeVertexBuffer(Arrays.stream(zigZagDeltaVertexDictionary).boxed().collect(Collectors.toList()),
                    physicalLevelTechnique);
            numStreams+=2;
            return Triple.of(numStreams, CollectionUtils.concatByteArrays(encodedTopologyStreams, encodedVertexOffsetStream,
                    encodedVertexDictionaryStream), maxVertexValue);
        }
        else{
            var encodedMortonVertexOffsetStream = IntegerEncoder.encodeIntStream(mortonEncodedDictionaryOffsets,
                    physicalLevelTechnique, false, PhysicalStreamType.OFFSET, new LogicalStreamType(OffsetType.VERTEX));
            var encodedMortonEncodedVertexDictionaryStream = IntegerEncoder.encodeMortonStream(new ArrayList<>(mortonEncodedDictionary),
                    zOrderCurve.numBits(), zOrderCurve.coordinateShift(), physicalLevelTechnique);
            numStreams+=2;
    ...
    private static void sortLineStrings(List<Long> featureIds, List<Integer> numParts,
        ...
        * */
        ...
        for(var numPart : numParts){
            var currentLineVertexOffsets = mortonEncodedDictionaryOffsets.subList(partOffsetCounter, partOffsetCounter + numPart);
            ...
            if(sortedDictionaryOffsets.containsKey(minVertexOffset)){
                ...
                sortedDictionaryOffsets.put(minVertexOffset, Triple.of(Lists.newArrayList(featureId),
                        currentLineVertexOffsets, Lists.newArrayList(numPart)));
            }
        }

        var sortedOffsets = sortedDictionaryOffsets.values().stream().flatMap(e -> e.getMiddle().stream()).toList();
        var updatedFeatureIds = sortedDictionaryOffsets.values().stream().flatMap(e -> e.getLeft().stream()).toList();
        var updatedNumParts = sortedDictionaryOffsets.values().stream().flatMap(e -> e.getRight().stream()).toList();
    ...
    private static byte[] encodeVertexBuffer(List<Integer> values, PhysicalLevelTechnique physicalLevelTechnique){
        ...
        var encodedMetadata = new StreamMetadata(PhysicalStreamType.DATA, new LogicalStreamType(DictionaryType.VERTEX),
                ...
                values.size(), encodedValues.length).encode();

...
public class StringDecoder {
            ...
            switch (streamMetadata.physicalStreamType()){
                case LENGTH:{
                    if(LengthType.DICTIONARY.equals(streamMetadata.logicalStreamType().lengthType())){
                        dictionaryLengthStream = IntegerDecoder.decodeIntStream(data, offset, streamMetadata, false);
                    }
                    else{
    ...
    public static Triple<Integer, BitSet, List<String>> decode(byte[] data, IntWrapper offset, int numStreams,
                                                               BitSet presentStream, int numValues) throws IOException {
        /*
        ...
        * */
        ...
        for(var i = 0; i < numStreams; i++){
            ...
            switch (streamMetadata.physicalStreamType()){
                ...
                case DATA:{
                    var ds = Arrays.copyOfRange(data, offset.get(), offset.get() + streamMetadata.byteLength());
        ...
        if(symbolTableStream != null){
            ...
            return Triple.of(numValues, presentStream, decodePlain(presentStream, dictionaryLengthStream, dataStream, numValues));
        }
    }

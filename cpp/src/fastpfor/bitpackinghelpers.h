...
inline void fastunpack(const uint32_t* __restrict__ in, uint64_t* __restrict__ out, const uint32_t bit) {
    // Could have used function pointers instead of switch.
    // Switch calls do offer the compiler more opportunities for optimization in
    // theory. In this case, it makes no difference with a good compiler.
    switch (bit) {
...
inline void fastpack(const uint64_t* __restrict__ in, uint32_t* __restrict__ out, const uint32_t bit) {
    switch (bit) {
        case 0:
            __fastpack0(in, out);
            break;
...
inline void fastpackwithoutmask(const uint64_t* __restrict__ in, uint32_t* __restrict__ out, const uint32_t bit) {
    switch (bit) {
        case 0:
            __fastpackwithoutmask0(in, out);
            break;
...
uint32_t* packblockup(const IntType* source, uint32_t* out, const uint32_t bit) {
...
template <uint32_t BlockSize, typename IntType = uint32_t>
const uint32_t* unpackblock(const uint32_t* source, IntType* out, const uint32_t bit) {
    for (uint32_t j = 0; j != BlockSize; j += 32) {

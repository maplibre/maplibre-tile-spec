...
public:
    ...
    static bool tryme(const uint32_t* n, size_t len) {
        ...
        const uint32_t minv = (static_cast<uint32_t>(len) < num1) ? static_cast<uint32_t>(len) : num1;
    ...
    static void bit_writer(uint64_t* out, const uint32_t value, const uint32_t bits) { *out = (*out << bits) | value; }

    enum {
        SIMPLE8B_LOGDESC = 4,
        SIMPLE8B_LEN = (1U << SIMPLE8B_LOGDESC)
    };

    static uint32_t which(const uint64_t* const in) { return static_cast<uint32_t>((*in) >> (64 - SIMPLE8B_LOGDESC)); }
    void encodeArray(const uint32_t* in, size_t length, uint32_t* out, size_t& nvalue) override;

    // like encode Array, but does not write the data
    void fakeencodeArray(const uint32_t* in, size_t length, size_t& nvalue);
    /*
     * we allow a buffer overrun of 240 elements, so out should be
     * able to accomodate the full uncompress data plus 240 elements.
     */
    const uint32_t* decodeArray(const uint32_t* in, size_t len, uint32_t* out, size_t& nvalue) override;
    std::string name() const override { return "Simple8b"; }
    Simple8b() = default;
    ...
    static void unpack(uint32_t*& out, const uint64_t*& in) {
        if (log1 < 32) {
            ...
            for (uint32_t k = 0; k < num1; ++k) {
                *(out++) = static_cast<uint32_t>(in[0] >> (64 - SIMPLE8B_LOGDESC - log1 - (k * log1))) & mask;
            }
        } else {
            for (uint32_t k = 0; k < num1; ++k) {
                *(out++) = static_cast<uint32_t>(in[0] >> (64 - SIMPLE8B_LOGDESC - log1 - (k * log1)));
            }
        }
        ++in;
    }
    template <uint32_t log1>
    static void carefulunpack(uint32_t num1, uint32_t*& out, const uint64_t*& in) {
        if (log1 < 32) {
            const uint32_t actuallog1 = log1 < 32 ? log1 : 31; // bogus to silence compiler warnings
            const uint32_t mask = ((static_cast<uint32_t>(1) << actuallog1) - 1);
            for (uint32_t k = 0; k < num1; ++k) {
...
template <bool MarkLength>
void Simple8b<MarkLength>::encodeArray(const uint32_t* in, const size_t length, uint32_t* out, size_t& nvalue) {
#ifndef NDEBUG
...
template <bool MarkLength>
void Simple8b<MarkLength>::fakeencodeArray(const uint32_t* in, const size_t length, size_t& nvalue) {
    uint32_t NumberOfValuesCoded = 0;
    size_t writtenout = 0;
    size_t ValuesRemaining(length);
    while (ValuesRemaining > 0) {

...
auto decodeVarints(BufferStream& buffer) {
    ...
    else return std::tuple_cat(std::move(v), decodeVarints<T, N - 1>(buffer));
}
...
void decodeVarints(BufferStream& buffer, std::size_t numValues, TTarget* out) {
    offset_t dstOffset = 0;
    for (std::size_t i = 0; i < numValues; ++i) {
        out[dstOffset++] = static_cast<TTarget>(decodeVarint<TDecode>(buffer));
    }
}

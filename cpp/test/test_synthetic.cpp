...
const std::vector<ExceptionPattern>& binaryExceptionPatterns() {
    static const std::vector<ExceptionPattern> patterns = {
        ...
        {.pattern = std::regex{"^fpf_align_\\d+$"}, .reason = "sub-optimal java vertex encoding"},
        {.pattern = std::regex{"^props_u32_fpf_\\d+$"}, .reason = "sub-optimal java vertex encoding"},
    };
    return patterns;
}
...
#warning "MLT_WITH_JSON is disabled; skipping decoded semantic comparison and relying on byte-level comparison only"
    ...
    if (!entry.binaryExceptionReason.empty()) {
        GTEST_SKIP() << "Byte equality not checked for '" << name << "': " << entry.binaryExceptionReason
                     << " (fixture: " << fileContents.size() << ", generated: " << generated.bytes.size() << " bytes)";
    }

    // Write the result to a file for debugging
    const auto failedFilePath = basePath + name + ".failed";
    writeBinaryFile(failedFilePath, generated.bytes);
    FAIL() << "Generated bytes do not match fixture file for '" << name
           << "'. Generated bytes written to: " << failedFilePath;
}

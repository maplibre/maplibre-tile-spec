...
public:
     ...
     */
    virtual void encodeArray(const uint32_t* in, size_t length, uint32_t* out, size_t& nvalue) = 0;

    virtual void encodeArray(const uint64_t*, const size_t, uint32_t*, size_t&) {
     ...
     */
    virtual const uint32_t* decodeArray(const uint32_t* in, size_t length, uint32_t* out, size_t& nvalue) = 0;

    virtual const uint32_t* decodeArray(const uint32_t*, const size_t, uint64_t*, size_t&) {
        throw std::logic_error("Not implemented!");
    }
     ...
     */
    virtual std::vector<uint32_t> uncompress(const std::vector<uint32_t>& compresseddata,
                                             size_t expected_uncompressed_size) {
        std::vector<uint32_t> data(expected_uncompressed_size); // allocate plenty of memory
        size_t memavailable = data.size();
        try {
            decodeArray(compresseddata.data(), compresseddata.size(), data.data(), memavailable);
...
public:
  ...
  static void fakeencodeArray(const uint32_t * /*in*/, const size_t length,
                       size_t &nvalue) {
    nvalue = length;
  }
  ...
  std::string name() const override { return "JustCopy"; }
};

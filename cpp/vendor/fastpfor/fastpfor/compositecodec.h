...
public:
    ...
    std::string name() const override { return codec1.name() + "+" + codec2.name(); }

private:
    template <typename IntType>
    void _encodeArray(const IntType* in, const size_t length, uint32_t* out, size_t& nvalue) {
        if (nvalue == 0) {
            return;
        }
        ...
        if (roundedlength < length) {
            if (nvalue1 > nvalue) {
                throw std::logic_error("Encode run over output buffer. Potential buffer overflow!");
            }
    ...
    template <typename IntType>
    const uint32_t* _decodeArray(const uint32_t* in, const size_t length, IntType* out, size_t& nvalue) {
        if (nvalue == 0) {
            return in;
        }

...
template <uint32_t BlockSizeInUnitsOfPackSize = 8, /* BlockSizeInUnitsOfPackSize can have value 4 or 8 */
          typename IntType = uint32_t /* Integer type, currently support uint32_t or uint64_t */>
class FastPForImpl {
public:
    /**
    ...
    const uint32_t* decodeArray(const uint32_t* in, const size_t length, IntType* out, size_t& nvalue) {
        ...
        while (out != finalout) {
            size_t thisnvalue(0);
            auto thissize = static_cast<size_t>(finalout > PageSize + out ? PageSize : (finalout - out));

            __decodeArray(in, thisnvalue, out, thissize); // NOLINT(readability-suspicious-call-argument)
            in += thisnvalue;
            out += thissize;
        }
     ...
     */
    void encodeArray(const IntType* in, const size_t length, uint32_t* out, size_t& nvalue) {
        ...
        *out++ = static_cast<uint32_t>(length);
        ...
        nvalue = 1;
        while (in != finalin) {
            auto thissize = static_cast<size_t>(finalin > PageSize + in ? PageSize : (finalin - in));
            size_t thisnvalue(0);
        ...
        if (oldnvalue < nvalue) {
            std::string msg;
            msg =
                "It is possible we have a buffer overrun. You reported having "
                "allocated " +
                std::to_string(oldnvalue * sizeof(uint32_t)) + " bytes for the compressed data but we needed " +
    ...
    void getBestBFromData(const IntType* in, uint8_t& bestb, uint8_t& bestcexcept, uint8_t& maxb) {
    ...
    void __encodeArray(const IntType* in, const size_t length, uint32_t* out, size_t& nvalue) {
        ...
        for (const IntType* const final = in + length; (in + BlockSize <= final); in += BlockSize) {
            ...
            *bc++ = bestcexcept;
            if (bestcexcept > 0) {
                *bc++ = maxb;
                std::vector<IntType>& thisexceptioncontainer = datatobepacked[maxb - bestb];
                const auto maxval = static_cast<IntType>(1ULL << bestb);
                for (uint32_t k = 0; k < BlockSize; ++k) {
                    if (in[k] >= maxval) {
        ...
        const auto bytescontainersize = static_cast<uint32_t>(bc - bytescontainer.data());
        *(out++) = bytescontainersize;
        memcpy(out, bytescontainer.data(), bytescontainersize);
        uint8_t* pad8 = reinterpret_cast<uint8_t*>(out) + bytescontainersize;
        out += (bytescontainersize + sizeof(uint32_t) - 1) / sizeof(uint32_t); // NOLINT(bugprone-sizeof-expression)
        while (pad8 < reinterpret_cast<uint8_t*>(out)) {
        ...
        *(reinterpret_cast<IntType*>(out)) = bitmap;
        out += (sizeof(IntType) + sizeof(uint32_t) - 1) / sizeof(uint32_t); // NOLINT(bugprone-sizeof-expression)
    ...
    // NOLINTNEXTLINE(bugprone-reserved-identifier)
    void __decodeArray(const uint32_t* in, size_t& length, IntType* out, const size_t nvalue) {
        const uint32_t* const initin = in;
        ...
        const auto* bytep = reinterpret_cast<const uint8_t*>(inexcept);
        inexcept += (bytesize + sizeof(uint32_t) - 1) / sizeof(uint32_t); // NOLINT(bugprone-sizeof-expression)
        IntType bitmap = *(reinterpret_cast<const IntType*>(inexcept));
        inexcept += (sizeof(IntType) + sizeof(uint32_t) - 1) / sizeof(uint32_t); // NOLINT(bugprone-sizeof-expression)
        for (uint32_t k = 2; k <= sizeof(IntType) * 8; ++k) {
        ...
        typename std::vector<IntType>::const_iterator unpackpointers[(sizeof(IntType) * 8) + 1];
        ...
        for (uint32_t run = 0; run < nvalue / BlockSize; ++run, out += BlockSize) {
            ...
            if (cexcept > 0) {
                ...
                if (maxbits - b == 1) {
                    ...
                    typename std::vector<IntType>::const_iterator& exceptionsptr = unpackpointers[maxbits - b];
                    for (uint32_t k = 0; k < cexcept; ++k) {
                        const uint8_t pos = *(bytep++);
...
public:
    FastPFor(uint32_t pageSize = 65536)
        : pfor32(pageSize) {}
    ...
    std::string name() const override { return std::string("FastPFor") + std::to_string(BlockSize); }
...
public:
    ...
    void __decodeArray(const uint32_t* in, size_t& length, uint32_t* out, const size_t nvalue) {
        ...
        inexcept = ecoder.decodeArray(inexcept, le, datatobepacked.data(), cap);

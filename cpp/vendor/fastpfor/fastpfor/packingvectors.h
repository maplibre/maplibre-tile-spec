...
public:
    ...
    static const uint32_t* unpackme(const uint32_t* in, STLContainer& out, const uint32_t bit) {
    ...
    static const uint32_t* unpackmetight(const uint32_t* in, IntType* out, size_t outSize, const uint32_t bit) {
        const uint32_t size = *in;
        ++in;
        uint32_t j = 0;
    ...
    template <class STLContainer>
    static uint32_t* packmeupwithoutmask(STLContainer& source, uint32_t* out, const uint32_t bit) {
        const size_t size = source.size();
        *out = static_cast<uint32_t>(size);
        out++;
        if (source.size() == 0) {
    ...
    template <typename IntType>
    static uint32_t* packmeuptightwithoutmask(const IntType* source, size_t size, uint32_t* out, const uint32_t bit) {
        *out = static_cast<uint32_t>(size);
        out++;
        uint32_t j = 0;

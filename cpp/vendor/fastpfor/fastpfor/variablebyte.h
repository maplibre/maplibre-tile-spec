...
public:
    ...
    template <typename T>
    void _encodeArray(const T* in, const size_t length, uint32_t* out, size_t& nvalue) {
        auto* bout = reinterpret_cast<uint8_t*>(out);
    ...
    template <typename T>
    void encodeToByteArray(const T* in, const size_t length, uint8_t* bout, size_t& nvalue) {
    ...
    const uint32_t* decodeArray(const uint32_t* in, const size_t length, uint32_t* out, size_t& nvalue) override {
        decodeFromByteArray<uint32_t>(reinterpret_cast<const uint8_t*>(in), length * sizeof(uint32_t), out, nvalue);
        return in + length;
    }

    const uint32_t* decodeArray(const uint32_t* in, const size_t length, uint64_t* out, size_t& nvalue) override {
        decodeFromByteArray<uint64_t>(reinterpret_cast<const uint8_t*>(in), length * sizeof(uint32_t), out, nvalue);
        return in + length;
    }

    template <typename T>
    const uint8_t* decodeFromByteArray(const uint8_t* inbyte, const size_t length, T* out, size_t& nvalue) {
        if (length == 0) {
            nvalue = 0;
            return inbyte; // abort
        }
...
public:
    ...
    static void encodeToByteArray(const uint32_t* in, const size_t length, uint8_t* bout, size_t& nvalue) {
        const uint8_t* const initbout = bout;
        for (size_t k = 0; k < length; ++k) {
    ...
    const uint32_t* decodeArray(const uint32_t* in, const size_t length, uint32_t* out, size_t& nvalue) override {
        decodeFromByteArray(reinterpret_cast<const uint8_t*>(in), length * sizeof(uint32_t), out, nvalue);
        return in + length;
    }

    static const uint8_t* decodeFromByteArray(const uint8_t* inbyte,
                                              const size_t length,
                                              uint32_t* out,
                                              size_t& nvalue) {
        if (length == 0) {

...
v2 uses the same data model as [v1](specification.md): tiles, layers, features, columns and streams as described in the [Overview](overview.md).
...
Unchanged from [v1](specification.md#tile-layout).
...
The meaning of the topology streams, the length threshold rules, componentwise delta coding, Hilbert-sorted vertex dictionaries and Morton codes are unchanged from [v1](specification.md#geometry-column).
...
`0xC`-`0xE` are unassigned and MUST be rejected.
...
|---|---|---|---|---|---|---|
| **Int** | Lengths, offsets, ids, integer columns, geometry topology | None | Delta | RLE | DeltaRLE | BitPacked |
| **Str** | A string column's leading stream | None | Delta | RLE | DeltaRLE | BitPacked |
| **Bool** | A boolean column's data stream | None | RLE† | | | |
...
Properties that are maps or lists with string keys, nestable.

v2 will shred nested values as [ORC](https://orc.apache.org/specification/ORCv1/) and Parquet do.
A nested value is decomposed into one flat column per leaf path.
Each leaf column is encoded like any other column.
The structure is stored separately from the values.

v1 sketched a `ComplexColumn` type with children, struct/list/map physical types and a control-value data stream.
It was not implemented, and v2 does not reuse that design.

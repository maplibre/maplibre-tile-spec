...
| `0x1`-`0x6` | It has none: every feature has [geometry type](#geometry-types) `code - 1` |
| `0x7` | Reserved, MUST be rejected |

A layer of one geometry type is the common case, and its types stream is a single run that says nothing this nibble cannot, so the nibble carries the type and the stream is not written.
A uniform type is held to the same rule as a types stream's values: it MUST agree with the topology the [geometry layout](#geometry-layout) declares.
...
When the header byte's m-value bit is clear, `column_counts` is `column_count` as a plain varint.
...
| Bits | Field |
|---|---|
| 7-4 | Number of shared presence bitfields, `0`-`14` |
| 3-0 | [Geometry layout](#geometry-layout) |

A column names a bitfield through its [presence nibble](#presence-nibble), which reaches index `13`, so a count of `15` declares a bitfield no column could read and MUST be rejected.
...
| `2`-`15` | The layer's shared bitfield at index `nibble - 2`. |
...
=== "Shared presence"
    ...
    Two columns referencing one shared bitfield, which the layout byte counts.

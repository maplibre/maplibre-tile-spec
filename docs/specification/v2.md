...
| | v1 | v2 |
|---|---|---|
| Extent | `varint`, any non-zero value | One nibble, a power of two in `32`-`32768` |
...
body := [string name]                    non-empty, UTF-8, VarInt length prefix
        [u8 extent]                      reserved nibble, extent code, see Extent
...
v2 stores only power-of-two extents, so the extent is one nibble rather than a varint.
...
The code is `log2(extent) - 5`:

| Code | `0x0` | `0x1` | `0x2` | `0x3` | `0x4` | `0x5` | `0x6` | `0x7` | `0x8` | `0x9` | `0xA` |
|---|---|---|---|---|---|---|---|---|---|---|---|
| Extent | 32 | 64 | 128 | 256 | 512 | 1024 | 2048 | 4096 | 8192 | 16384 | 32768 |

Codes `0xB`-`0xF` are unassigned and MUST be rejected, as MUST a non-zero high nibble.

The upper bound keeps every coordinate of a tile inside the 16 bits a [Morton](#parameters) code is laid on.

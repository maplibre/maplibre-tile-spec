...
| Bits | Field |
|---|---|
| 7-4 | Number of shared presence fields, `0`-`12` |
| 3-0 | [Geometry layout](#geometry-layout) |

A column names a field through its [presence nibble](#presence-nibble), which reaches index `11`, so a count of `13` or more declares a field no column could read and MUST be rejected.
...
```
shared_presence := [u8 coding]   1, 2 or 3, see Presence Codings
                   [payload]     per that coding, self-delimiting
```

The `n` shared fields follow the layout byte back to back, in index order.
A shared field has no presence nibble to carry its coding, so it names one itself.
...
| `4`-`15` | The layer's shared field at index `nibble - 4`. |

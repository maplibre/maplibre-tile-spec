...
A [corpus](#shared-string-leaves) carries a name field that names nothing: a decoder ignores it, and it MAY repeat any name.
...
Nibbles `4`-`15` are reserved and MUST be rejected.

Kinds `2` and `3` are a corpus alone, for leaves elsewhere to index, so they write no child count and no children.
They appear in a [nested column's corpus section](#shared-string-leaves), which is the only place anything indexes them.
An encoder MUST NOT write one as a counted column, where nothing can.
...
nested_column := [u8 column_type]        presence nibble over 0xC, 0xD or 0xE
                 ...
                 corpus_section          only when a leaf below indexes a corpus
```
...
A presence nibble of `2` or `3` over any data type but `0xB` MUST be rejected: only a string leaf holds codes.
...
| `2` | As `0`, for a [shared string leaf](#shared-string-leaves). |
| `3` | As `1`, for a [shared string leaf](#shared-string-leaves). |
| `4`-`15` | Reserved, MUST be rejected. |
...
It is a stream rather than the raw bitfield a column writes, because under a [`List`](#list-nodes) or a [`Map`](#map-nodes) the number of bits is not known until the lengths have been decoded, and the stream header lets a decoder step over it either way.
...
A string leaf whose node presence nibble is `2` or `3` is the one exception: its dictionary lives in the column's [corpus section](#shared-string-leaves) instead of beside its codes.

### Shared String Leaves

A string leaf holds a whole string stream set, so a struct's string fields each pay a corpus, a dictionary-length stream and, under FSST, a symbol table, where the same values as counted columns would share one [shared dictionary](#shared-dictionary-columns).
Node presence nibbles `2` and `3` give a string leaf that reach: its codes index a corpus its column holds, and the leaf stores nothing but them.

```
shared_leaf := [u8 node_type]          presence nibble 2 or 3 over data type 0xB
               [string name]           only a struct field has one
               [presence stream]       only when the presence nibble is 3
               [varint corpus_index]   which of the column's corpora the codes index
               [codes stream]          one corpus entry index per present value
```

The codes are the `Codes` stream of the [`Dict` string layout](#string-columns), so the leading stream's extension bits are `01` as they would be on a leaf holding its own dictionary.
The streams that layout ends with are the corpus, which every leaf indexing it shares.

The corpora a column's leaves index trail its body:

```
corpus_section := [varint corpus_count]    non-zero
                  corpus * corpus_count
corpus         := [u8 column_type]         low nibble 0xF, high nibble 2 or 3
                  [string name]            an encoder hint, which no decoder reads
                  [corpus streams]         per the dictionary kind the high nibble names
```

A corpus is a [shared dictionary column](#shared-dictionary-columns) of a corpus-only kind, read exactly as one, and front coding of its entries is named by the encoding byte of its last stream as it is there.

The section is present only when some leaf below the root indexes a corpus, so a column that shares nothing writes the bytes it wrote before this section existed.
A `corpus_index` at or past `corpus_count` MUST be rejected, as MUST a shared leaf in a column with no corpus section.

Corpora belong to the column that writes them, not to the layer, and two columns MAY hold corpora of the same entries.
This is what lets an encoder compare a `Struct` and its `Map` rewrite by stored size: each candidate writes only the corpora its own leaves read, rather than one of the two externalising its dictionary.
...
stream := [u8 encoding_byte]
          ...
          [varint byte_length]  unless the physical field is `00`, see Byte Length
...
`byte_length` is present unless the [physical field](#physical-field) is `00`.
That pattern is assigned only to the logical `None` encoding, whose payload is the elements as they are, and says the length follows from the value count and the element width the stream's type fixes:
...
Every other logical encoding MUST reject it.
...
=== "Shared corpus"
    ...
    Three `0x2B` string fields whose codes index one `0x2F` corpus trailing the column body.
    None of the three carries a dictionary of its own.

    ```
    --8<-- "0x02/nested_shared_dict.mlt.hexdump"
    ```

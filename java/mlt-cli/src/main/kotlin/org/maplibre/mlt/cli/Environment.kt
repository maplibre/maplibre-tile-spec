...
val compressionRatioThreshold by lazy {
    parse(ENV_COMPRESSION_RATIO_THRESHOLD, DEFAULT_COMPRESSION_RATIO_THRESHOLD, String::toDouble).also {
        if (it < 0.0) {
...
val tileLogInterval by lazy {
    parse(ENV_TILE_LOG_INTERVAL, DEFAULT_TILE_LOG_INTERVAL, String::toULong).let {
        // treat zero as "never"
...
val cacheMaxHeapPercent by lazy {
    parse(ENV_CACHE_MAX_HEAP_PERCENT, DEFAULT_CACHE_MAX_HEAP_PERCENT, String::toInt).also {
        if (it < 0 || it > 100) {
...
val cacheExpireAfterAccess by lazy {
    parse(ENV_CACHE_EXPIRE, DEFAULT_CACHE_EXPIRE, Duration::parse).also {
        if (it.isNegative()) {
...
val threadQueueSize by lazy {
    parse(ENV_THREAD_QUEUE_SIZE, DEFAULT_THREAD_QUEUE_SIZE, String::toInt).also {
        if (it < 1) {

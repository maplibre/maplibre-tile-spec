...
class TileCoordRangeTest {
    ...
    fun `allows zero offset byte range`() {
        val range = ReadablePmtiles.TileCoordRange(42L, 3, ByteRange(0, 7))
    ...
    fun `rejects negative offset byte range`() {
        assertThrows(IllegalArgumentException::class.java) {
            ReadablePmtiles.TileCoordRange(0L, 1, ByteRange(-1, 7))
        }
    }
    ...
    fun `rejects non-positive tile count`() {
        assertThrows(IllegalArgumentException::class.java) {
            ReadablePmtiles.TileCoordRange(0L, 0, ByteRange(0, 7))
        }
    }
}

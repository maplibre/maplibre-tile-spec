...
public class PropertyEncoder {
  ...
  public static ArrayList<byte[]> encodePropertyColumns(
    ...
    for (var columnMetadata : propertyColumns) {
      ...
      if (columnMetadata.isScalar()) {
        ...
        encodedColumn = encodeMapPropertyColumn(features, useFSST, columnMetadata, physicalLevelTechnique, integerEncodingOption);
  ...
  private static @Nullable Boolean getBooleanPropertyValue(@NotNull Feature feature, @NotNull String name) {
    return feature
        .findProperty(name)
  ...
  private static @Nullable Float getFloatPropertyValue(@NotNull Feature feature, @NotNull String name) {
    final var index = feature.getIndex();
    return feature
        .findProperty(name)
        .map(
  ...
  private static @Nullable Double getDoublePropertyValue(@NotNull Feature feature, @NotNull String name) {
    final var index = feature.getIndex();
    return feature
        .findProperty(name)
        .map(
  ...
  private static ArrayList<byte[]> encodeStringColumn(
    ...
    if (columnMetadata.isNullable()) {
      final var presentValues =
          Arrays.stream(rawStringValues).map(Objects::nonNull);
      presentStream = BooleanEncoder.encodeBooleanStream(rawStringValues.length, presentValues::iterator, PhysicalStreamType.PRESENT);
    } else {
  ...
  private static ArrayList<byte[]> encodeMapPropertyColumn(
          ...
          MltMetadata.Column columnMetadata,
          PhysicalLevelTechnique physicalLevelTechnique,
          @NotNull ConversionConfig.IntegerEncodingOption encodingOption)
          throws IOException {
    ...
    // Now that all the values are collected and their final order established, assign indexes for encoding
    ...
    encodedStreams.add(new byte[] { (byte)uniqueValues.dictionaryPresenceMask() });
    ...
    if (!uniqueValues.uniqueBooleanValues().isEmpty()) {
      encodedStreams.addAll(
          BooleanEncoder.encodeBooleanStream(
              uniqueValues.uniqueBooleanValues().keySet(),
              PhysicalStreamType.DATA));
      numStreams++;
    }
    ...
    if (!uniqueValues.uniqueInt64Values().isEmpty()) {
      encodedStreams.addAll(
          IntegerEncoder.encodeLongStream(uniqueValues.uniqueInt64Values().keySet(),
              true,
              PhysicalStreamType.DATA,
              null, encodingOption));
      numStreams++;
    ...
    if (flattenedMapData.anyEmpty()) {
      encodedStreams.addAll(
      BooleanEncoder.encodeBooleanStream(
              flattenedMapData.featurePresentValues(),
              PhysicalStreamType.PRESENT));
      numStreams++;
    }
  ...
  enum SpecialMapValue {
    NULL(0),
    START_MAP(1), /// indicates the value is a nested map rather than a scalar, followed by nested payload length and payload values
    START_LIST(2), /// indicates the value is a list rather than a scalar, followed by payload length and payload values
    COUNT(3);

    SpecialMapValue(int value) {
      this.value = value;
    }
    final int value;
  }

  /// Holds the sorted unique values for each encodable type for a map property column and their corresponding indexes
  // TODO: Better to combine [u]int32/64 values and encode everything as 64-bit if anything needs it?
  private record UniqueMapValues(
      @NotNull TreeMap<String, Integer> uniqueStringValues,
      @NotNull TreeMap<Boolean, Integer> uniqueBooleanValues,
    ...
    private static final int MASK_STRING = 1;
    private static final int MASK_BOOLEAN = 1 << 1;
    private static final int MASK_INT32 = 1 << 2;
    private static final int MASK_UINT32 = 1 << 3;
    private static final int MASK_INT64 = 1 << 4;
    private static final int MASK_UINT64 = 1 << 5;
    private static final int MASK_FLOAT = 1 << 6;
    private static final int MASK_DOUBLE = 1 << 7;
    ...
    void assignIndexes() {
      var start = SpecialMapValue.COUNT.value;
      start = assignIndexes(uniqueStringValues, start);
      start = assignIndexes(uniqueBooleanValues, start);
    ...
    int dictionaryPresenceMask() {
      return 
          ((!uniqueStringValues.isEmpty() ? MASK_STRING : 0)
              | (!uniqueBooleanValues.isEmpty() ? MASK_BOOLEAN : 0)
  ...
  boolean anyEmpty, boolean allEmpty) {}
  ...
  private static FlattenedMapData flattenMapValues(
    ...
    return new FlattenedMapData(flattenedValues, featureValueCounts, featurePresentValues, anyEmpty, allEmpty);
  }
  ...
  private static void appendMapEntryValue(
      ...
      @NotNull final String columnName) {
    switch (value) {
      case null -> flattenedValues.add(SpecialMapValue.NULL.value);
      case Map<?, ?> nestedMap -> {
        appendMapValue(nestedMap, flattenedValues, uniqueValues, columnName);
      }
      case Iterable<?> iterable -> appendListValue(iterable, flattenedValues, uniqueValues, columnName);
  ...
  private static void appendMapValue(
      ...
      @NotNull final String columnName) {
    final var startIndex = flattenedValues.size();
    flattenedValues.add(SpecialMapValue.START_MAP.value);
  ...
  private static void appendListValue(
      ...
      @NotNull final String columnName) {
    final var startIndex = flattenedValues.size();
    flattenedValues.add(SpecialMapValue.START_LIST.value);
    flattenedValues.add(0); // size not known yet, will be updated after payload is added
  ...
  private static void appendListItemValue(
      ...
      @NotNull final String columnName) {
    switch (item) {
      case null -> listValueIndexes.add(SpecialMapValue.NULL.value);
      case Map<?, ?> nestedMap -> appendMapValue(nestedMap, listValueIndexes, uniqueValues, columnName);
      case Iterable<?> nestedList -> appendListValue(nestedList, listValueIndexes, uniqueValues, columnName);
  ...
  private static int getScalarIndex(
      @NotNull final Object value, @NotNull final UniqueMapValues uniqueValues, @NotNull final String columnName) {
    return switch (value) {
      case Boolean boolValue ->
        getScalarIndex(uniqueValues.uniqueBooleanValues(), boolValue, columnName);
      case U8 u8Value ->
        getScalarIndex(uniqueValues.uniqueUInt32Values(), U32.of(u8Value.intValue()), columnName);
      case Integer intValue -> getScalarIndex(uniqueValues.uniqueInt32Values(), intValue, columnName);
      case U32 u32Value -> getScalarIndex(uniqueValues.uniqueUInt32Values(), u32Value, columnName);
      case Long longValue -> getScalarIndex(uniqueValues.uniqueInt64Values(), longValue, columnName);
      case U64 u64Value -> getScalarIndex(uniqueValues.uniqueUInt64Values(), u64Value, columnName);
      case Float floatValue -> getScalarIndex(uniqueValues.uniqueFloatValues(), floatValue, columnName);
      case Double doubleValue -> getScalarIndex(uniqueValues.uniqueDoubleValues(), doubleValue, columnName);
      case String stringValue -> getScalarIndex(uniqueValues.uniqueStringValues(), stringValue, columnName);
      default ->
          throw new IllegalArgumentException(
  ...
  private static <T> int getScalarIndex(@NotNull final Map<T, Integer> indexes, @NotNull final T value, @NotNull final String columnName) {
  ...
  /// Recursively walk a nested map property value, collecting all the unique scalars used as key and leaf values.
  /// Indexes cannot be established yet, so all are set to zero.
  private static void collectUniqueMapValues(
      @Nullable final Object value, @NotNull final UniqueMapValues uniqueValues, @NotNull final String columnName) {
    switch (value) {
      case null -> {
      ...
      case Boolean boolValue -> uniqueValues.uniqueBooleanValues().putIfAbsent(boolValue, 0);
      case U8 u8Value -> uniqueValues.uniqueUInt32Values().putIfAbsent(U32.of(u8Value.intValue()), 0);

...
public class SyntheticMltGenerator {
  ...
  private static void generateSharedDictionaries() throws IOException {
    ...
    var feat_neq_names_eq_val = array(feat(p0, props(kv("a", val), kv("b", val))));
    write(layer("props_shared_dict_no_struct_name", feat_neq_names_eq_val), cfg().sharedDictPrefix("", ""));
    write(layer("props_shared_dict_no_struct_name_fsst", feat_neq_names_eq_val), cfg().sharedDictPrefix("", "").fsst());

    var feat_names_eq_val_eq = array(feat(p0, props(kv("a", val), kv("a", val))));
    write(layer("props_shared_dict_no_child_name", feat_names_eq_val_eq), cfg().sharedDictPrefix("a", ""));
    write(layer("props_shared_dict_no_child_name_fsst", feat_names_eq_val_eq), cfg().sharedDictPrefix("a", "").fsst());
  }
}

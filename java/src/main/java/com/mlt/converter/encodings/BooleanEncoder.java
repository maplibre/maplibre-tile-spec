...
public class BooleanEncoder {
    ...
    * */
    public static byte[] encodeBooleanStream(List<Boolean> values, PhysicalStreamType streamType) throws IOException {
        ...
        var valuesMetadata = new StreamMetadata(streamType, null, LogicalLevelTechnique.RLE, LogicalLevelTechnique.NONE,
                PhysicalLevelTechnique.NONE,  values.size(), encodedValueStream.length).encode();
     ...
     * Combines a BitVector encoding with the Byte RLE encoding form the ORC format
     * */
    public static byte[] encodeBooleanStreamOptimized(List<Boolean> values, PhysicalStreamType streamType) throws IOException {
        var valueStream = new BitSet(values.size());
        for(var i = 0; i < values.size(); i++){
            var value = values.get(i);
            valueStream.set(i, value);

...
public class PropertyEncoder {
    ...
    public static byte[] encodePropertyColumns(List<MltTilesetMetadata.Column> propertyColumns, List<Feature> features,
                                               boolean useAdvancedEncodings, Optional<List<ColumnMapping>> columnMappings) throws IOException {
        /**
         * TODOs:
         * - detect if column is nullable to get rid of the present stream
         * - test boolean rle against roaring bitmaps and integer encoding for present stream and boolean values
         * - Add vector type to field metadata
         */

        var physicalLevelTechnique = useAdvancedEncodings ? PhysicalLevelTechnique.FAST_PFOR : PhysicalLevelTechnique.VARINT;
        var featureScopedPropertyColumns = new byte[0];

        var i = 0;
        for(var columnMetadata : propertyColumns){
            if(columnMetadata.hasScalarType()) {
                var encodedScalarPropertyColumn = encodeScalarPropertyColumn(columnMetadata, features,
                        physicalLevelTechnique, useAdvancedEncodings);
                featureScopedPropertyColumns = CollectionUtils.concatByteArrays(featureScopedPropertyColumns, encodedScalarPropertyColumn);
            }
            else if(columnMetadata.hasComplexType() && columnMetadata.getComplexType().getPhysicalType()
                    == MltTilesetMetadata.ComplexType.STRUCT){
                    if(!columnMappings.isPresent()){
                        throw new IllegalArgumentException("Column mappings are required for nested property columns.");
                    }
                    ...
                    for (var nestedFieldMetadata : columnMetadata.getComplexType().getChildrenList()) {
                        if (nestedFieldMetadata.getScalarField().getPhysicalType() == MltTilesetMetadata.ScalarType.STRING) {
    ...
    public static byte[] encodeScalarPropertyColumn(MltTilesetMetadata.Column columnMetadata, List<Feature> features,
                                                    PhysicalLevelTechnique physicalLevelTechnique, boolean useAdvancedEncodings) throws IOException {
        ...
        switch (scalarType) {
            ...
            case UINT_32: {
                var intColumn = encodeInt32Column(features, columnMetadata.getName(), physicalLevelTechnique, false);
                var encodedFieldMetadata = EncodingUtils.encodeVarints(new long[]{2}, false, false);
                return CollectionUtils.concatByteArrays(encodedFieldMetadata, intColumn);
            }
            case INT_32: {
                var intColumn = encodeInt32Column(features, columnMetadata.getName(), physicalLevelTechnique, true);
                var encodedFieldMetadata = EncodingUtils.encodeVarints(new long[]{2}, false, false);
                return CollectionUtils.concatByteArrays(encodedFieldMetadata, intColumn);
            }
            case UINT_64: {
            ...
            case STRING: {
                ...
                var stringColumn = StringEncoder.encode(values, physicalLevelTechnique, useAdvancedEncodings);
            ...
            default:
                throw new IllegalArgumentException("The specified scalar data type is currently not supported.");
        }
    }

    private static byte[] encodeBooleanColumn(List<Feature> features, String fieldName) throws IOException {
        ...
        var encodedPresentStreamMetadata = new StreamMetadata(PhysicalStreamType.PRESENT, null,
                LogicalLevelTechnique.RLE, LogicalLevelTechnique.NONE, PhysicalLevelTechnique.NONE,
                presentStreamIndex, encodedPresentStream.length).encode();
        var encodedDataStreamMetadata = new StreamMetadata(PhysicalStreamType.DATA, null,
                LogicalLevelTechnique.RLE, LogicalLevelTechnique.NONE, PhysicalLevelTechnique.NONE,
                dataStreamIndex, encodedDataStream.length).encode();
    ...
    private static byte[] encodeInt32Column(List<Feature> features, String fieldName, PhysicalLevelTechnique physicalLevelTechnique,
        ...
        var present = new ArrayList<Boolean>(features.size());
        for(var feature : features){
            var propertyValue = fieldName.equals(ID_COLUMN_NAME)? feature.id() : feature.properties().get(fieldName);
            if(propertyValue != null){
                //TODO: refactor -> handle long values for ids differently
                var intValue = propertyValue instanceof Long? ((Long) propertyValue).intValue():
                        (int)propertyValue;
                values.add(intValue);
                present.add(true);
            }

...
public class IntegerDecoder {
    ...
    public static List<Integer> decodeMortonStream(byte[] data, IntWrapper offset, MortonEncodedStreamMetadata streamMetadata){
        int[] values;
    ...
    public static List<Integer> decodeIntStream(byte[] data, IntWrapper offset, StreamMetadata streamMetadata, boolean isSigned){
        ...
        var decodedValues = decodeIntArray(values, streamMetadata.logicalLevelTechnique2(), streamMetadata, isSigned, false);
        //TODO: get rid of that conversion
        //TODO: find proper solution with zig-zag encoding -> currently only second level technique is allowed to use
        //zig-zag encoding
        return decodeIntArray(decodedValues.stream().mapToInt(i -> i).toArray(), streamMetadata.logicalLevelTechnique1(),
                streamMetadata, isSigned, true);
    }

    private static List<Integer> decodeIntArray(int[] values, LogicalLevelTechnique logicalLevelTechnique,
                                                StreamMetadata streamMetadata, boolean isSigned, boolean isSecondPass){
        switch (logicalLevelTechnique){
            case RLE:{
                ...
                return isSigned && !isSecondPass? decodeZigZag(decodedValues.stream().mapToInt(i -> i).toArray()) : decodedValues;
            }
            case DELTA:
                return isSecondPass && isSigned? decodeDelta(values) : decodeZigZagDelta(values);
            case NONE:{
                return isSigned && !isSecondPass? decodeZigZag(values) : Arrays.stream(values).boxed().collect(Collectors.toList());
            }
            case MORTON:
                //TODO: zig-zag decode when morton second logical level technique
                return decodeMortonCodes(values, ((MortonEncodedStreamMetadata)streamMetadata).numBits(),
    ...
    public static List<Long> decodeLongStream(byte[] data, IntWrapper offset, StreamMetadata streamMetadata, boolean isSigned){
        ...
        var decodedValues = decodeLongArray(values, streamMetadata.logicalLevelTechnique1(), streamMetadata, isSigned);
        //TODO: get rid of that conversion
        return decodeLongArray(decodedValues.stream().mapToLong(i -> i).toArray(), streamMetadata.logicalLevelTechnique2(),
                streamMetadata, isSigned);
    }

    private static List<Long> decodeLongArray(long[] values, LogicalLevelTechnique logicalLevelTechnique,
                                                StreamMetadata streamMetadata, boolean isSigned){
        switch (logicalLevelTechnique){
            case RLE:{
                var rleMetadata = (RleEncodedStreamMetadata)streamMetadata;
                var decodedValues = decodeRLE(values, rleMetadata.runs(), rleMetadata.numRleValues());

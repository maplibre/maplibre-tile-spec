...
public abstract class Vector<T extends Buffer, K> {
    ...
    public Vector(String name, BitVector nullabilityBuffer, T dataBuffer) {
        this.name = name;
        this.nullabilityBuffer = nullabilityBuffer != null ? Optional.of(nullabilityBuffer) : Optional.empty();
    ...
    public int size() {
        ...
        return this.nullabilityBuffer.isPresent() ? this.nullabilityBuffer.get().size() : this.dataBuffer.capacity();
    }

 ...
 *
 * <p>(c) Daniel Lemire, http://lemire.me/en/ (c) Intel Corp. (for Vector implementation)
 */
...
/**
 * This is a patching scheme designed for speed. It encodes integers in blocks of integers within
 * pages of up to 65536 integers. Note that it is important, to get good compression and good
 * performance, to use sizeable arrays (greater than 1024 integers). For arrays containing a number
 * of integers that is not divisible by BLOCK_SIZE, you should use it in conjunction with another
 * CODEC:
 *
 * <p>IntegerCODEC ic = new Composition(new VectorFastPFOR(), new VariableByte()).
 *
 * <p>For details, please see:
 *
 * <p>Daniel Lemire and Leonid Boytsov, Decoding billions of integers per second through
 * vectorization Software: Practice &amp; Experience <a
 ...
 * schemes. Note that this does not use differential coding: if you are working on sorted lists, you
 * should first compute deltas, @see me.lemire.integercompression.differential.Delta#delta.
 *
 * <p>For multi-threaded applications, each thread should use its own FastPFOR object.
 *
 * @author Daniel Lemire
 */
public class VectorFastPFOR implements IntegerCODEC, SkippableIntegerCODEC {
  private static final int OVERHEAD_OF_EACH_EXCEPT = 8;
  public static final int DEFAULT_PAGE_SIZE = 64 << 10;

  public static final int BLOCK_SIZE = 256;
  private static final int INTS_PER_BLOCK = BLOCK_SIZE >>> 5;
   ...
   *
   * @param pagesize the desired page size (recommended value is FastPFOR.DEFAULT_PAGE_SIZE)
   */
  ...
  public VectorFastPFOR() {
    this(DEFAULT_PAGE_SIZE);
  }

  /**
   * Compress data in blocks of BLOCK_SIZE integers (if fewer than BLOCK_SIZE integers are provided,
   * nothing is done).
   *
   * @see IntegerCODEC#compress(int[], IntWrapper, int, int[], IntWrapper)
   */
  @Override
  public void headlessCompress(
      int[] in, IntWrapper inpos, int inlength, int[] out, IntWrapper outpos) {
  ...
  private void getBestBitSize(int[] in, int pos, int index) {
    ...
    bem[index] = 32;
    while (freqs[bem[index]] == 0) bem[index]--;
    ...
    for (int b = bem[index] - 1; b >= 0; --b) {
      ...
      int thiscost = cexcept * OVERHEAD_OF_EACH_EXCEPT + cexcept * (maxb - b) + b * BLOCK_SIZE + 8;
      ...
      if (tmpexcept > 0) {
        ...
        for (int k = 0; k < BLOCK_SIZE; ++k) {
          if ((in[k + tmpinpos] >>> tmpbestb) != 0) {
            ...
            bem[bindex++] = (byte) k;
            dataTobePacked[index][dataPointers[index]++] = in[k + tmpinpos] >>> tmpbestb;
          }
        }
      } else {
        bindex += 2;
    ...
    for (int k = 2; k <= 32; ++k) {
      if (dataPointers[k] != 0) {
        ...
        for (; j < n; j += BLOCK_SIZE) {
          VectorBitPacker.fastpackNoMask(dataTobePacked[k], j, out, tmpoutpos, k);
          tmpoutpos += INTS_PER_BLOCK * k;
        }
        int r = dataPointers[k] % BLOCK_SIZE;
        if (r != 0) {
          tmpoutpos = VectorBitPacker.slowpack(dataTobePacked[k], j, r, out, tmpoutpos, k);
          tmpoutpos++;
        }
      }
    }
  ...
  /**
   * Uncompress data in blocks of integers. In this particular case, the inlength parameter is
   * ignored: it is deduced from the compressed data.
   *
   * @see IntegerCODEC#compress(int[], IntWrapper, int, int[], IntWrapper)
   */
  @Override
  public void headlessUncompress(
      int[] in, IntWrapper inpos, int inlength, int[] out, IntWrapper outpos, int mynvalue) {
  ...
  private void decodePage(int[] in, IntWrapper inpos, int[] out, IntWrapper outpos, int thissize) {
    ...
    for (int k = 2; k <= 32; ++k) {
      if ((bitmap & (1 << (k - 1))) != 0) {
        ...
        if (dataTobePacked[k].length < roundedup) dataTobePacked[k] = new int[roundedup];
        if (inexcept + roundedup / 32 * k <= in.length) {
          ...
          inexcept = VectorBitPacker.slowunpack(in, inexcept, dataTobePacked[k], j, r, k);
        } else {
          int j = 0;
          ...
          for (; j < l; j += BLOCK_SIZE) {
            VectorBitPacker.fastunpack(buf, inexcept - initinexcept, dataTobePacked[k], j, k);
            inexcept += INTS_PER_BLOCK * k;
          }
          int r = size % BLOCK_SIZE;
          inexcept = VectorBitPacker.slowunpack(in, inexcept, dataTobePacked[k], j, r, k);
    ...
    int tmpinpos = inpos.get();
    int idx = 0;
    for (int run = 0, run_end = thissize / BLOCK_SIZE;
        run < run_end;
   ...
   *
   * <p>In case you need a different way to allocate buffers, you can override this method with a
   * custom behavior. The default implementation allocates a new Java direct {@link ByteBuffer} on
   * each invocation.
   */

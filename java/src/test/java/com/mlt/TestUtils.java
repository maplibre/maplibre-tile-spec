...
public class TestUtils {
  ...
  private static int compareFeatures(
    ...
    int numErrors = 0;
    assertEquals(mvtFeatures.size(), mltFeatures.size(), "mvtFeatures.size()=" + mvtFeatures.size() + " mltFeatures.size()=" + mltFeatures.size());
    for (var j = 0; j < mltFeatures.size(); j++) {
      ...
      var mvtFeature =
          isFeatureTableSorted
              ...
              : mvtFeatures.get(j);
      assertEquals(mvtFeature.id(), mltFeature.id(), "mvtFeature.id()=" + mvtFeature.id() + " mltFeature.id()=" + mltFeature.id());
      ...
      for (var mvtProperty : mvtProperties.entrySet()) {
        ...
        if (mltProperty == null) {
          ...
          assertEquals(mvtProperty.getValue(), mltProperty, "mvtProperty.getValue()=" + mvtProperty.getValue() + " mltProperty=" + mltProperty);
        }
      }
    }
    return numErrors;
  ...
  public static int compareTilesVectorized(
    ...
    assertEquals(featureTables.length, mvtLayers.size(), "featureTables.length=" + featureTables.length + " mvtLayers.size()=" + mvtLayers.size());
  ...
  public static int compareTilesSequential(
    ...
    var mvtLayers = mvTile.layers();
    assertEquals(mltLayers.size(), mvtLayers.size(), "mltLayers.size()=" + mltLayers.size() + " mvtLayers.size()=" + mvtLayers.size());
    for (var i = 0; i < mvtLayers.size(); i++) {
      var mvtLayer = mvtLayers.get(i);
      var mvtFeatures = mvtLayers.get(i).features();

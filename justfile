...
test-java-cli:
    ...
    python3 -c 'import os; expected=2534; ts=os.path.getsize("output/varint.mlt.meta.pbf"); assert ts == expected, f"tile size changed from expected ({expected}), got: {ts}"'
    ...
    python3 -c 'import os; expected=65006; ts=os.path.getsize("output/advanced.mlt"); assert ts == expected, f"tile size changed from expected ({expected}), got: {ts}"'

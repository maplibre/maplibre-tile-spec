...
pub fn compress_fsst_with<S: AsRef<str>>(
    ...
    // Compress all strings as one concatenated buffer. This allows FSST symbol
    // matches across string boundaries, matching Java's approach. The decoder
    // decompresses the full corpus and splits by original (uncompressed) value lengths.

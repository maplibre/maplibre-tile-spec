...
///
/// `params.shift` is added to both axes to move the origin into the non-negative
/// range (use the global minimum across *all* vertices for the layer, not
/// per-axis).  `params.bits` is the grid level — both shifted components must
/// fit in `[0, 2^bits)`.
///
/// Use [`hilbert_curve_params_from_bounds`] to compute `shift` and `bits`
/// from global min/max coordinates.
#[must_use]
pub fn hilbert_sort_key(c: Coord32, params: GridParams) -> u32 {
    ...
    hilbert_xy_to_index(params.bits, sx, sy)
}
...
///
/// Returns a [`GridParams`] where:
/// - `shift` equals `min_val.unsigned_abs()` when `min_val < 0`, else `0`,
///   ensuring all shifted coordinates are non-negative.
/// - `bits` is the smallest level `l` in `[1, 16]` such that all
///   shifted values fit in `[0, 2^l)`.
///
/// If `min > max` (empty input), returns `GridParams { bits: 1, shift: 0 }`.
#[must_use]
pub fn hilbert_curve_params_from_bounds(min_val: i32, max_val: i32) -> GridParams {
    if min_val > max_val {
        return GridParams { bits: 1, shift: 0 };
    }
...
mod tests {
    ...
    fn hilbert_sort_key_origin_zero() {
        assert_eq!(
            hilbert_sort_key(c(0, 0), GridParams { shift: 0, bits: 1 }),
            0
        );
    }
    ...
    fn hilbert_sort_key_negative_coords_shift_correctly() {
        ...
        assert_eq!(
            hilbert_sort_key(c(-1, -1), GridParams { shift: 1, bits: 1 }),
            0
        );
    }
    ...
    fn hilbert_sort_key_matches_xy_to_index() {
        ...
        // after shifting.
        let params = GridParams { shift: 5, bits: 4 };
        for raw_x in -5i32..11 {
            for raw_y in -5i32..11 {
                let expected = hilbert_xy_to_index(
                    params.bits,
                    u32::try_from(i64::from(raw_x) + i64::from(params.shift)).unwrap(),
                    u32::try_from(i64::from(raw_y) + i64::from(params.shift)).unwrap(),
                );
                let actual = hilbert_sort_key(c(raw_x, raw_y), params);
    ...
    fn curve_params_empty_bounds() {
        ...
        let p = hilbert_curve_params_from_bounds(i32::MAX, i32::MIN);
        assert_eq!(p.shift, 0);
        assert_eq!(p.bits, 1);
    }
    ...
    fn curve_params_all_zero() {
        ...
        let p = hilbert_curve_params_from_bounds(0, 0);
        assert_eq!(p.shift, 0);
        assert_eq!(p.bits, 1);
    }
    ...
    fn curve_params_positive_only() {
        ...
        let p = hilbert_curve_params_from_bounds(0, 3);
        assert_eq!(p.shift, 0);
        assert_eq!(p.bits, 2);
    }
    ...
    fn curve_params_negative_min() {
        ...
        let p = hilbert_curve_params_from_bounds(-4, 4);
        assert_eq!(p.shift, 4);
        assert_eq!(p.bits, 4);
    }
    ...
    fn curve_params_power_of_two_extent() {
        ...
        let p = hilbert_curve_params_from_bounds(0, 8);
        assert_eq!(p.shift, 0);
        assert_eq!(p.bits, 4);
    }
    ...
    fn curve_params_single_axis_negative() {
        ...
        let p = hilbert_curve_params_from_bounds(-2, 5);
        assert_eq!(p.shift, 2);
        assert_eq!(p.bits, 3);
    }
    ...
    fn curve_params_clamped_at_16_bits() {
        ...
        let p = hilbert_curve_params_from_bounds(0, 65535);
        assert_eq!(p.shift, 0);
        assert_eq!(p.bits, 16);
    }

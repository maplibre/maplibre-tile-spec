...
use crate::decoder::stream::header02::{
    Count02, HAS_EXPLICIT_COUNT, StrLayout, StreamCtx02, is_packed_bitmap,
};
...
fn parse_corpora<'a>(
    ...
    // Each corpus requires at least 1 byte (column type).
    if input.len() < count.into_usize() {
        return Err(MltError::BufferUnderflow(count, input.len()));
    }

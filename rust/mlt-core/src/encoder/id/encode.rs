...
use crate::encoder::stream::write::{write_bool_stream, write_u32_stream_as, write_u64_stream_as};
use crate::encoder::stream::{DataProfile, IntEncoder, LogicalEncoder};
...
impl StagedId {
    ...
    #[hotpath::measure]
    pub fn write_to(self, enc: &mut Encoder, codecs: &mut Codecs) -> MltResult<bool> {
        ...
        if has_nulls {
            ...
            match &self {
                Self::OptId { presence, .. } => {
                    write_bool_stream(presence.iter().copied(), StreamType::Present, enc, codecs)?;
                }
                Self::Id(_) => {
                    // override_presence requested a stream for an all-present column
                    write_bool_stream(
                        std::iter::repeat_n(true, feature_count),
                        StreamType::Present,
                        enc,
                        codecs,
                    )?;
        ...
        if let Some(single_enc) = single_enc {
            write_id_value_stream(&self, id_width, single_enc, enc, codecs)?;
        } else {
            ...
            for &cand in &candidates {
                alt.with(|enc| write_id_value_stream(&self, id_width, cand, enc, codecs))?;
            }
        }

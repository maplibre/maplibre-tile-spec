...
struct WriteTrait<std::string> {
    static inline capi::DiplomatWrite Construct(std::string& t) { return diplomat::WriteFromString(t); }
};
...
public:
    ...
    ~result() = default;
    bool is_ok() const { return std::holds_alternative<Ok<T>>(this->val); }
    bool is_err() const { return std::holds_alternative<Err<E>>(this->val); }
    ...
    std::optional<std::reference_wrapper<std::remove_reference_t<E>>> err() && {
        ...
        return std::make_optional(
            std::reference_wrapper(std::forward<E>(std::get<Err<E>>(std::move(this->val)).inner)));
    }

    void set_ok(T&& t) { this->val = Ok<T>(std::move(t)); }

    void set_err(E&& e) { this->val = Err<E>(std::move(e)); }
...
public:
    ...
    constexpr T* data() const noexcept { return this->data_; }
...
private:
    using capi_type =
        std::conditional_t<std::is_same_v<value_type, char>,
                           ...
                           std::conditional_t<std::is_same_v<value_type, char16_t>, capi::DiplomatString16View, void>>;
...
replace_fn_t<T> replace_ret(T val) {
    if constexpr (std::is_same_v<T, std::string_view>) {
        ...
        // Can we convert straight away to our slice type, or (in the case of ABI compatible structs), do we have to do
        // a reinterpret cast?
...
template <typename T>
struct inner { /* only T*, std::optional, and std::unique_ptr are supported */
};
template <typename T>
struct inner<T*> {
    using type = T;
...
struct next_to_iter_helper {
    ...
    bool operator!=(std::nullopt_t) { return (bool)_curr; }

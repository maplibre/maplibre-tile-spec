...
mod tests {
    ...
    #[tokio::test]
    async fn auto_preserves_input_pmtiles_gzip_compression() {
        let input = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../test/fixtures/omt-planet-20260112.mvt.max1.pmtiles");
        let output = TempOutput::new();
        ...
        let reader = PmReader::new_with_cached_path(HashMapCache::default(), &output.0)
            .await
            .expect("output opens");
        assert_eq!(reader.get_header().tile_compression, Compression::Gzip);
    }

    async fn make_compressed_mvt_fixture(compression: Compression) -> TempOutput {
        let fixture = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../test/fixtures/omt-planet-20260112.mvt.max1.pmtiles");
        let reader = Arc::new(
            PmReader::new_with_cached_path(HashMapCache::default(), fixture)
                .await
                .expect("fixture opens"),
        );
        let tile_id = collect_pmtiles_ids(&reader)
            .await
            .expect("fixture directory reads")[0];
        let mvt = reader
            .get_tile_decompressed(tile_id)
            .await
            .expect("fixture tile decompresses")
            .expect("fixture tile exists");

        let output = TempOutput::new();
        let file = fs::File::create(&output.0).expect("temporary source is created");
        let mut writer = PmTilesWriter::new(TileType::Mvt)
            .tile_compression(compression)
            .create(file)
            .expect("temporary source writer is created");
        writer
            .add_tile(TileCoord::new(0, 0, 0).unwrap(), &mvt)
            .expect("compressed MVT tile is written");
        writer.finalize().expect("temporary source is finalized");
        output
    }

    #[tokio::test]
    async fn auto_preserves_brotli_and_zstd_pmtiles_compression() {
        for compression in [Compression::Brotli, Compression::Zstd] {
            let input = make_compressed_mvt_fixture(compression).await;
            let output = TempOutput::new();
            ...
            assert_eq!(reader.get_header().tile_compression, compression);

            let tile_id = collect_pmtiles_ids(&reader)
                .await
                .expect("output directory reads")[0];
            let tile = reader
                .get_tile_decompressed(tile_id)
                .await
                .expect("output tile decompresses")
                .expect("output tile exists");
            assert!(
                !Parser::default()
                    .parse_layers(&tile)
                    .expect("decompressed MLT tile parses")

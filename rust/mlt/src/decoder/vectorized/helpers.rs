...
pub fn get_vector_type_int_stream(metadata: &StreamMetadata) -> Result<VectorType, MltError> {
    ...
    match (tech1, tech2, runs, n) {
        ...
        (Some(LogicalLevelTechnique::Rle), _, Some(1), _) => Ok(VectorType::Const),
        (Some(LogicalLevelTechnique::Rle), _, Some(_), _) => Ok(VectorType::Flat),
        // L1 == DELTA && L2 == RLE && runs in {1,2} → SEQUENCE
        (Some(LogicalLevelTechnique::Delta), Some(LogicalLevelTechnique::Rle), Some(r), _)
            if r == 1 || r == 2 =>
        {
            Ok(VectorType::Sequence)
        }
        // num_values == 1 → CONST; else FLAT
        (_, _, _, 1) => Ok(VectorType::Const),
        _ => Ok(VectorType::Flat),
    }
}
...
mod tests {
    ...
    fn table_driven_vector_type_int_stream() {
        ...
        for (desc, meta, expected) in cases {
            let vt = get_vector_type_int_stream(&meta).unwrap();

...
#[test]
fn hovering_inside_a_layer_targets_a_feature() {
    let mut app = sample_app();
    press(&mut app, KeyCode::Down);
    let bounds = app.get_bounds();
    app.find_hovered_feature(250.0, 250.0, bounds);
    assert_eq!(
...
#[test]
fn hovering_inside_a_feature_targets_a_part() {
    let mut app = sample_app();
    press(&mut app, KeyCode::Down);
    press(&mut app, KeyCode::Char('+'));
    press(&mut app, KeyCode::Down);
    press(&mut app, KeyCode::Down);
    let bounds = app.get_bounds();
    app.find_hovered_feature(3320.0, 3320.0, bounds);
    assert_eq!(
        app.hovered.as_ref().map(|h| &h.item),
...
#[test]
fn map_clicks_select_the_hovered_item_one_level_at_a_time() {
    let mut app = sample_app();
    let bounds = app.get_bounds();
    app.find_hovered_feature(1000.0, 3000.0, bounds);
    assert_eq!(
        app.hovered.as_ref().map(|h| &h.item),
        Some(&TreeItem::Layer(2))
    );
    let hovered = app.hovered.clone().unwrap();
    app.handle_map_click(&hovered.item, HEIGHT);
    assert_eq!(app.selected_item(), &TreeItem::Layer(2));

    let bounds = app.get_bounds();
    app.find_hovered_feature(3000.0, 1000.0, bounds);
    let hovered = app.hovered.clone().unwrap();
    assert_eq!(hovered.item, TreeItem::Feature { layer: 2, feat: 1 });
    app.handle_map_click(&hovered.item, HEIGHT);
    assert_eq!(
        app.selected_item(),
        &TreeItem::Feature { layer: 2, feat: 1 }
    );

    let bounds = app.get_bounds();
    app.find_hovered_feature(3200.0, 1200.0, bounds);
    let hovered = app.hovered.clone().unwrap();
    assert_eq!(
        hovered.item,
        TreeItem::SubFeature {
...
#[test]
fn hovering_a_tree_row_targets_that_row() {
    let mut app = sample_app();
...
fn opening_another_file_drops_the_previous_hover_and_scroll() {
    ...
    app.hovered = Some(HoveredInfo {
        tree_idx: Some(last),
        item: app.tree_items[last].clone(),
    });

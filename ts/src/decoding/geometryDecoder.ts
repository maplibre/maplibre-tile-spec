...
import type {
  GeometryVector,
  MortonSettings,
} from "../vector/geometry/geometryVector";
...
export function decodeGeometryColumn(
  ...
  const geometryTypesVectorType = getVectorType(
  ...
  if (geometryTypesVectorType === VectorType.CONST) {
    ...
    const geometryType = decodeUnsignedConstInt32Stream(
      tile,
      offset,
      geometryTypeMetadata,
    );
      ...
      switch (geometryStreamMetadata.physicalStreamType) {
        ...
        case PhysicalStreamType.OFFSET: {
          switch (geometryStreamMetadata.logicalStreamType.offsetType) {
            case OffsetType.VERTEX:
              vertexOffsets = decodeUnsignedInt32Stream(
                tile,
                offset,
                geometryStreamMetadata,
              );
            ...
            case OffsetType.INDEX:
              indexBuffer = decodeUnsignedInt32Stream(
                tile,
                offset,
                geometryStreamMetadata,
              );
        ...
        case PhysicalStreamType.DATA: {
          if (
            ...
            const mortonMetadata =
              geometryStreamMetadata as MortonEncodedStreamMetadata;
            mortonSettings = {
              numBits: mortonMetadata.numBits,
              coordinateShift: mortonMetadata.coordinateShift,
            };
            vertexBuffer = decodeUnsignedInt32Stream(
              tile,
              offset,
              geometryStreamMetadata,
              scalingData,
  ...
  const geometryTypeVector = decodeUnsignedInt32Stream(
    tile,
    offset,
    geometryTypeMetadata,
  );
  ...
  for (let i = 0; i < numStreams - 1; i++) {
    ...
    switch (geometryStreamMetadata.physicalStreamType) {
      case PhysicalStreamType.LENGTH:
        switch (geometryStreamMetadata.logicalStreamType.lengthType) {
          case LengthType.GEOMETRIES:
            geometryLengths = decodeUnsignedInt32Stream(
              tile,
              offset,
              geometryStreamMetadata,
            );
            break;
          case LengthType.PARTS:
            partLengths = decodeUnsignedInt32Stream(
              tile,
              offset,
              geometryStreamMetadata,
            );
  ...
  if (geometryLengths) {
    geometryOffsets = decodeRootLengthStream(
      geometryTypeVector,
      geometryLengths,
      2,
    );
    if (partLengths && ringLengths) {
      partOffsets = decodeLevel1LengthStream(
        geometryTypeVector,
        geometryOffsets,
        partLengths,
        false,
      );
      ringOffsets = decodeLevel2LengthStream(
        geometryTypeVector,
        geometryOffsets,
        partOffsets,
        ringLengths,
      ...
      partOffsets = decodeLevel1WithoutRingBufferLengthStream(
        geometryTypeVector,
        geometryOffsets,
        partLengths,
      );
    ...
    ringOffsets = decodeLevel1LengthStream(
      geometryTypeVector,
      partOffsets,
      ringLengths,
      true,
  ...
  return mortonSettings ===
    undefined /* Currently only 2D coordinates (Vec2) are implemented in the encoder  */
    ? createFlatGeometryVector(
        geometryTypeVector,
        { geometryOffsets, partOffsets, ringOffsets },
        vertexOffsets,
...
function decodeRootLengthStream(
  ...
  for (let i = 0; i < geometryTypes.length; i++) {
    ...
    previousOffset = rootBufferOffsets[i + 1] =
      previousOffset +
      (geometryTypes[i] > bufferId ? rootLengthStream[rootLengthCounter++] : 1);
  }
...
function decodeLevel1LengthStream(
  ...
  const level1BufferOffsets = new Uint32Array(
    rootOffsetBuffer[rootOffsetBuffer.length - 1] + 1,
  );
  let previousOffset = 0;
...
function decodeLevel2LengthStream(
  ...
  const level2BufferOffsets = new Uint32Array(
    level1OffsetBuffer[level1OffsetBuffer.length - 1] + 1,
  );
  let previousOffset = 0;

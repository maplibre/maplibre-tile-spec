...
export function decodeUnsignedIntStream(
    ...
    const values = reinterpretAsUint32(decodePhysicalLevelTechnique(data, offset, streamMetadata));
...
export function decodeConstIntStream(
    ...
    const values = decodePhysicalLevelTechnique(data, offset, streamMetadata);
    const unsignedValues = reinterpretAsUint32(values);

    if (values.length === 1) {
        return isSigned ? decodeZigZagInt32Value(values[0]) : unsignedValues[0];
    }

    return isSigned ? decodeZigZagConstRleInt32(values) : unsignedValues[1];
}
...
function decodeUnsignedInt32(
    ...
    const decodedValues = decodeInt32(
        reinterpretAsInt32(values),
        streamMetadata,
        false,
        scalingData,
        nullabilityBuffer,
...
function reinterpretAsInt32(values: Uint32Array): Int32Array {

...
describe("decodePropertyColumn - STRING", () => {
  describe("basic functionality", () => {
    it("should decode single field with shared dictionary", () => {
      ...
      const { lengthStream, dataStream } =
        encodeSharedDictionary(dictionaryStrings);
      const fieldStreams = encodeStructField(
        [0, 1, 2, 3],
        [true, true, true, true],
      );
      ...
      const columnMetadata = createColumnMetadataForStruct("address:", [
        { name: "street" },
      ]);
      const offset = new IntWrapper(0);
      const result = decodePropertyColumn(

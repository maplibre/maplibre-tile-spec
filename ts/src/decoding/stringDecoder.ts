  ...
  for (let i = 0; i < numStreams; i++) {
    ...
    switch (streamMetadata.physicalStreamType) {
      case PhysicalStreamType.PRESENT: {
        const presentData = decodeBooleanRle(
          data,
          streamMetadata.numValues,
          streamMetadata.byteLength,
          offset,
        );
        const presentStream = new BitVector(
          presentData,
          streamMetadata.numValues,
        );
      ...
      case PhysicalStreamType.LENGTH: {
        const lengthStream = decodeLengthStreamToOffsetBuffer(
          data,
          offset,
          streamMetadata,
        );
        if (
          LengthType.DICTIONARY === streamMetadata.logicalStreamType.lengthType
        ) {
          dictionaryLengthStream = lengthStream;
        } else if (
          LengthType.SYMBOL === streamMetadata.logicalStreamType.lengthType
        ) {
      ...
      case PhysicalStreamType.DATA: {
        const dataStream = data.subarray(
          offset.get(),
          offset.get() + streamMetadata.byteLength,
        );
        ...
        if (DictionaryType.FSST === dictType) {
          ...
          DictionaryType.SINGLE === dictType ||
          DictionaryType.SHARED === dictType
        ) {
...
function decodePlainStringVector(
  ...
  if (
    nullabilityBuffer &&
    nullabilityBuffer.size() !== plainLengthStream.length - 1
  ) {
...
export function decodeSharedDictionary(
    ...
    switch (streamMetadata.physicalStreamType) {
      ...
      case PhysicalStreamType.DATA:
        if (
          ...
          dictionaryBuffer = data.subarray(
            offset.get(),
            offset.get() + streamMetadata.byteLength,
          );
          dictionaryStreamDecoded = true;
        } else {
          symbolTableBuffer = data.subarray(
            offset.get(),
            offset.get() + streamMetadata.byteLength,
          );
        }
  ...
  for (const childField of childFields) {
    ...
    const columnName = childField.name

...
export function encodeIntStream(
    ...
    const { data } = isSigned
        ? encodeSignedInt32(values, metadata, bitVector, scalingData)
        : encodeUnsignedInt32(new Uint32Array(values.buffer, values.byteOffset, values.length), metadata, bitVector, scalingData);

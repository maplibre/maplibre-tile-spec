...
function decodeVarintRemainder(l, buf, offset) {
    let h, b;
    b = buf[offset.get()];  offset.increment(); h  = (b & 0x70) >> 4;  if (b < 0x80) return h * 0x100000000 + (l >>> 0);
    b = buf[offset.get()];  offset.increment();h |= (b & 0x7f) << 3;  if (b < 0x80) return h * 0x100000000 + (l >>> 0);
    b = buf[offset.get()];  offset.increment();h |= (b & 0x7f) << 10; if (b < 0x80) return h * 0x100000000 + (l >>> 0);
    b = buf[offset.get()];  offset.increment();h |= (b & 0x7f) << 17; if (b < 0x80) return h * 0x100000000 + (l >>> 0);
    b = buf[offset.get()];  offset.increment();h |= (b & 0x7f) << 24; if (b < 0x80) return h * 0x100000000 + (l >>> 0);
    b = buf[offset.get()];  offset.increment();h |= (b & 0x01) << 31; if (b < 0x80) return h * 0x100000000 + (l >>> 0);

    throw new Error("Expected varint not more than 10 bytes");
}

export function decodeFastPfor(data: Uint8Array, numValues: number, byteLength: number, offset: IntWrapper): Int32Array{
...
export function decodeRleInt64(data: BigInt64Array, streamMetadata: RleEncodedStreamMetadata, isSigned: boolean) : BigInt64Array {
    return isSigned
...
export function decodeRleFloat64(data: Float64Array, streamMetadata: RleEncodedStreamMetadata, isSigned: boolean) : Float64Array {
    return isSigned
...
export function decodeUnsignedRleInt64(encodedData: BigInt64Array, numRuns: number, numTotalValues: number): BigInt64Array {
...
export function decodeUnsignedRleFloat64(encodedData: Float64Array, numRuns: number, numTotalValues: number): Float64Array {
...
export function decodeNullableRle(data: Int32Array, streamMetadata: StreamMetadata, isSigned: boolean, bitVector: BitVector ): Int32Array{
...
export function decodeNullableRleInt64(data: BigInt64Array, streamMetadata: StreamMetadata, isSigned: boolean, bitVector: BitVector ): BigInt64Array{
    const rleMetadata = streamMetadata as RleEncodedStreamMetadata;
    return isSigned

  ...
  while (offset.get() < tile.length) {
    ...
    for (const columnMetadata of featureTableMetadata.columns) {
      ...
      if (isLogicalIdColumn(columnMetadata)) {
        ...
        numFeatures = nullabilityBuffer
          ? nullabilityBuffer.size()
          : idDataStreamMetadata.decompressedCount;
        ...
        const numStreams = columnHasStreamCount
          ? decodeVarintInt32(tile, offset, 1)[0]
          : 1;
...
function decodeIdColumn(
  ...
  if (
    ...
    throw new Error(
      `ID column must be a logical ID scalar type: ${columnName}`,
    );
  }

  const idDataType = scalarTypeMetadata.longID
    ? ScalarType.UINT_64
    : ScalarType.UINT_32;
  const nullabilityBuffer =
    typeof sizeOrNullabilityBuffer === "number"
      ? undefined
  ...
  if (idDataType === ScalarType.UINT_32) {
    switch (vectorType) {
      case VectorType.FLAT: {
        const id = decodeUnsignedInt32Stream(
          tile,
          offset,
          idDataStreamMetadata,
          undefined,
      ...
      case VectorType.SEQUENCE: {
        const id = decodeSequenceInt32Stream(
          tile,
          offset,
          idDataStreamMetadata,
        );
      ...
      case VectorType.CONST: {
        const id = decodeUnsignedConstInt32Stream(
          tile,
          offset,
          idDataStreamMetadata,
        );
  ...
  switch (vectorType) {
    case VectorType.FLAT: {
      if (idWithinMaxSafeInteger) {
        const id = decodeUnsignedInt64AsFloat64Stream(
          tile,
          offset,
          idDataStreamMetadata,
        );
      ...
      const id = decodeUnsignedInt64Stream(
        tile,
        offset,
        idDataStreamMetadata,
        nullabilityBuffer,
    ...
    case VectorType.CONST: {
      const id = decodeUnsignedConstInt64Stream(
        tile,
        offset,
        idDataStreamMetadata,
      );
